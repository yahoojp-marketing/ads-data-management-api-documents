# LINEヤフー広告 ConversionAPIタグ (サーバーサイドGoogleタグマネージャー用)
本ドキュメントでは [サーバーサイドGoogleタグマネージャー](https://developers.google.com/tag-platform/tag-manager/server-side) のコミュニティテンプレートギャラリーから「LINEヤフー広告 ConversionAPIタグ」を利用してウェブページコンバージョンを実施する手順を示します。

ConversionAPI の詳細については [こちらのドキュメント](./conversion-api.md) をご参照ください。 

> [!IMPORTANT]
> LINEヤフー広告のConversion APIとLINE Conversion APIを統合した、より精度の高いAPIも提供しています。新規で導入する場合は、新しいAPIのご利用をおすすめします。なお、「LINEヤフー広告 ConversionAPIタグ」は今後提供終了を予定しております。提供終了時期については決まり次第お知らせいたします。  
...
LINEヤフー広告 ConversionAPIタグ では サーバーサイドGoogleタグマネージャー のコンテナに送信されたコンバージョンデータを収集しConversionAPIにリクエストすることでLINEヤフー広告へのコンバージョン測定を実施します。

以下のセットアップを実施してください。

- [こちら](https://developers.google.com/tag-platform/tag-manager/server-side/cloud-run-setup-guide) の「Set up server-side tagging」セクションなどをご確認の上、サーバーサイドGoogleタグマネージャーのセットアップをお願いします。
...
「LINEヤフー広告 ConversionAPIタグ」ではConversion APIを利用します。
[LINEヤフー広告ヘルプ](https://ads-help.yahoo-net.jp/s/article/H000044232?language=ja) および [Conversion APIのドキュメント](./conversion-api.md) をご確認の上、以下をご用意ください。
- Client ID
- ConversionAPI用のリクエストパラメータ

## セットアップ: LINEヤフー広告 ConversionAPIタグ

### セットアップ1: 変数の設定

本セクションでは「LINEヤフー広告 ConversionAPIタグ」の入力値として設定するための変数を定義します。    
...
1. Tagsタブの「Tags Fired」に「LINEヤフー広告 ConversionAPIタグ」を利用したタグが存在する
...
## LINEヤフー広告 ConversionAPIタグ の動作について
...
hashed_email および hashed_phone_number の入力値について、SHA-256でハッシュ化されていない場合は以下の通りハッシュ化が実行されます。

| パラメータ名                        | SHA-256ハッシュ値以外の値が入力された場合の動作                                                                                                          |
|-------------------------------|-------------------------------------------------------------------------------------------------------------|
| hashed_email                  | 入力された文字列が有効なメールアドレス形式であると判定された場合、文字列はハッシュ化され送信されます。それ以外の場合は無視されます。 |
| hashed_phone_number           | 入力された電話番号に `-`, `(`, `)` または半角スペースが含まれる場合は削除します。また、電話番号が 0 で始まる場合は日本の国番号 +81 に置換します。置換後の文字列がE.164形式の電話番号であると判定された場合、その文字列はハッシュ化され送信されます。それ以外の場合は無視されます。 |
